...
(function (root) {
  ...
  function occurrences(s,until=addDays(today(),14)){
    const out=[];
    const trackingStart=s.settings.trackingDate||'0001-01-01';
    for(const x of s.subscriptions.filter(x=>x.status==='active')){let due=x.nextDate;for(let n=0;due<trackingStart&&n<1200;n++)due=addMonths(due,x.cycle,x.anchorDay);for(let n=0;due<=until&&n<1200;n++,due=addMonths(due,x.cycle,x.anchorDay)){if(!hasPaid(s,'subscriptions',x.id,due))out.push({type:'subscriptions',id:x.id,name:x.name,amount:x.amount,currency:x.currency,date:due,period:due,reminder:x.reminder});}}
    for(const type of ['bills','debts']) for(const x of s[type].filter(x=>!x.archived&&(type!=='debts'||x.remaining>0))){const startMonth=month(x.startDate),trackingMonth=month(trackingStart);let m=[startMonth,trackingMonth,x.settledThrough?month(addMonths(`${x.settledThrough}-01`,1)):startMonth].sort().at(-1);let pending=0;for(let n=0;m<=month(until)&&n<1200;n++,m=month(addMonths(`${m}-01`,1))){const due=type==='debts'&&x.bank&&m===month(x.bank.endDate)?x.bank.endDate:dueInMonth(m,x.dueDay);if(type==='debts'&&x.bank&&due>x.bank.endDate)break;if(due<x.startDate||due<trackingStart||due>until||hasPaid(s,type,x.id,m)||(x.settledThrough&&m<=x.settledThrough))continue;const amount=type==='debts'?(x.bank?(m===month(x.bank.endDate)?x.bank.lastInstallment:x.installment):Math.min(x.installment,x.remaining-pending)):x.amount;if(amount<=0)break;pending+=amount;out.push({type,id:x.id,name:x.name,amount,currency:x.currency,date:due,period:m,reminder:x.reminder});}}
