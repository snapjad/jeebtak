...
(function (root) {
  ...
  function occurrences(s,until=addDays(today(),14)){
    ...
    for(const type of ['bills','debts']) for(const x of s[type].filter(x=>!x.archived&&(type!=='debts'||x.remaining>0))){let m=month(x.startDate);let pending=0;for(let n=0;m<=month(until)&&n<1200;n++,m=month(addMonths(`${m}-01`,1))){const due=dueInMonth(m,x.dueDay);if(due<x.startDate||due>until||hasPaid(s,type,x.id,m))continue;const amount=type==='debts'?Math.min(x.installment,x.remaining-pending):x.amount;if(amount<=0)break;pending+=amount;out.push({type,id:x.id,name:x.name,amount,currency:x.currency,date:due,period:m,reminder:x.reminder});}}
